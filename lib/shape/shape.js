...
import { fade, sunlight } from "../functions/light.js";
import {
    Vector,
...
import { rasterizeTriangle } from "../draw/draw.js";
...
export class Shape {
    ...
    pipeline(coords) {
        ...
        this.worldspace_translate_coords(coords); // model to world
        if (PERSPECTIVE) {
            this.perspective(coords);
        }
        this.canvas_translate_coords(coords); // world to canvas
    ...
    /**
     * @param {Number} surface_index index in Array coord_object.surfaces
     ...
     */
     draw_surface(surface_index, color, orthographic=!PERSPECTIVE, draw_normal=false) {
        ...
        let dot = dotProduct(v_to_surface, surface_ref.normal); console.log(dot);
        if (dot >= 0) {
            return;
        }
        ...
        // let corners = [surface_ref.corner1, surface_ref.corner2, surface_ref.corner3];
        // for (let i = 0; i < 3; i++) {
        //     let point_ref = corners[i];
        //     let display_coord = new Vector(point_ref.x, point_ref.y, point_ref.z);
        //     this.pipeline(display_coord);
        //     corners[i] = display_coord;
        // }
        // rasterizeTriangle(this.canvas, corners[0], corners[1], corners[2]);
...
export class CubeShape extends Shape {
    ...
    constructor(canvas, coord_object) {
        ...
        this.color_map = [0,0,0,0,0,0];
    }

    /**
     * 
     * @param {Array[Array[Number]]} colors [ [256, 256, 256], [100, 0, 0], ...] one rgb triple per surface
     * // gray 144, 144, 144
     */
    draw_surfaces(color=false) {
        ...
        let c;
        for (let i = 0; i < 6; i++) {
            c = color ? color[i] : this.colors[this.color_map[i]];
            this.draw_surface(i * 2, c);
            this.draw_surface(i * 2 + 1, c);
        }
    }
