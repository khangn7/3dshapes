...
export function drawline(x1, y1, x2, y2, gen=false) {
    ...
    if (mag_slope < 1) { 
        ...
        for (let x = start_x; x <= end_x; x++) {
            ...
            if (gen) {
                arr.push([x, start_y + p_y_change * sign]);
            } else {
...
export function rasterizeTriangle(c1, c2, c3) {
    ...
    if (flat_edge) {
        ...
        // note: y values in these arrays will be
        let r_points = drawline(right_c, odd_c, true); // [[x1, y1], [x2, y2], ...]
        let l_points = drawline(left_c, odd_c, true);
        if (r_points[0][1] != l_points[0][1]) {
            l_points = l_points.reverse();
        }

        let shorter, longer;
        if (r_points.length > l_points.length) {
            shorter = l_points;
            longer = r_points;
        } else {
            shorter = r_points;
        ...
        let previous_y = Infinity;
        for (let i = 0; i < longer.length; i++) {
            if (longer[i][1] == previous_y) {
                continue;
            }

            previous_y = longer[i][1];
